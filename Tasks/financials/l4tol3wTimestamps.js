...
db.getMongo().startSession().withTransaction(() => {
  try {
    ...
    if (bulkOps.length > 0) {
      ...
      print("[3] BulkWrite count:", bulkOps.length);
      atlasTimestamp.l3 = new Date();
    } else {
      print("[3] No documents to upsert into L3.");
      atlasTimestamp.l3 = new Date(); // still record time
    }
    ...
    auditColl.insertOne({
      ...
      sourceLastUpdateTime: payload.lastUpdatedDate,
    ...
    print("Transaction error caught inside block:", err.message);
    throw err; // force rollback
  }
});
